...
for csv_file in stat_files:
    ...
    if num_columns_existing:
        df[num_columns_existing] = df[num_columns_existing].applymap(lambda x: float(x.replace(",", ".")) if isinstance(x, str) else None if  pd.isna(x) else None if x is None else x)
