    ...
    for hit in response["hits"]["hits"]:  # Processing and Extracting Info Document-wise
        ...
        if "cac:ProcurementProject" in hit["_source"].keys(): ######## Processing for legacy XML ####################################
            ...
            try:
                ...
                cpv_data = project.get("cac:MainCommodityClassification",{}).get("cbc:ItemClassificationCode",0)
                ...
                ca_data = extract_contracting_authority(?)
                awards_data = extract_awarded_contracts(?)
                lot_data = extract_lots(?)
            except Exception as e:
